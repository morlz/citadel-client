		...
		<tr slot-scope="props" :key="props.itemKey" ref="trs" @click="rowClickHandler($event, props.item, props.itemIndex)">
			<td v-for="column, cIndex in table.columns" :title="column.getValue(props.item)" :key="cIndex">
				<div :style="column.style">
					<slot :name="column.field" :row="props.item" :column="column" :value="column.getValue(props.item)" :index="props.itemKey">
...
export default {
	...
	methods: {
		...
		checkForMore (index) {
			if (this.infinite.send || this.complete || this.noInfinite)
				return
